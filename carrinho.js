...
function atualizarPrecoTotal (produtoLinha) {
     ...
     const precoUnitario = 
parseFloat (produtoLinha.querySelector('.preco-unitario').innerText.replace('RS', 
'').replace(',', '.')); 
...
produtos.forEach (produtoLinha => {
    const precoTotal = 
    parseFloat (produtoLinha.querySelector('.preco-total').innerText.replace('RS', '').replace(',', '.')); 
